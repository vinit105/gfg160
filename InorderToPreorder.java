...
class Solution {
    public static Node buildTree(int inorder[], int preorder[]) {
        ...
        int[] index = new int[1];
        return fun(0,inorder.length-1, preorder, m,index);
    }
    static Node fun(int l, int r, int[] pre,HashMap<Integer, Integer> m, int[] index ){
        if(l>r ) return null;
        Node root = new Node(pre[index[0]++]);
        root.left = fun(l,m.get(root.data)-1, pre, m, index);
        root.right = fun(m.get(root.data)+1, r, pre, m,index);
        return root;
    }
}
